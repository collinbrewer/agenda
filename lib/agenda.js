...
var Agenda = module.exports = function(config, cb) {
  ...
  var self = this;
  var dbReady=function(err, collection){
    if(!err && config.logging){
     self.enableLogging();
    }
  ...
  if (config.mongo) {
    this.mongo(config.mongo, config.db ? config.db.collection : undefined, dbReady);
  } else if (config.db) {
    this.database(config.db.address, config.db.collection, config.db.options, dbReady);
  }
};
...
/** Setup and initialize the collection used to store Logs.
 * @param collection collection name or undefined for default 'agendaLogs'
 * CB 2016/06/25
 */
Agenda.prototype.enableLogging = function (collection) {
   var noop=function(){};
   this._logs=this._mdb.collection(collection || 'agendaLogs');
   this._logs.createIndex('job', noop)
   this._logs.createIndex('ranAt', noop);
};
...
Agenda.prototype.saveLog = function(job) {

   if(this._logs)
   {
      var props = {
        job: job.attrs.name,
        ranAt: job.attrs.lastRunAt,
        finishedAt: job.attrs.lastFinishedAt,
        data: job.attrs.data,
        result: job.attrs.failReason ? 'fail' : 'success',
        message: job.attrs.failReason ? job.attrs.failReason : job.attrs.message,
        logs: job.logs()
      };

      this._logs.insert(props, function(err, res) {
          if(err) throw(err);
      });
   }
};
