...
Job.prototype.run = function(cb) {
  ...
  setImmediate(function() {
    ...
    self.save(function() {
      var jobCallback = function(err, message) {
        if (err) {
          self.fail(err);
        }
        ...
        // logging
        self.attrs.message=(err ? err + "" : (message || ""));
        self.agenda.saveLog(self);
      };
...
Job.prototype.log = function (a) {
   var m;

   if(typeof(a)==="string"){
      var f=a.replace(/%o/g, "%j").replace(/%i|%f/g, "%d"); // convert console format to node util format
      m=util.format.apply(util, [f].concat([].slice.call(arguments, 1)));
   }
   else{
      m=util.inspect(a);
   }
